...
#define NODE_NAME "AT_CLIENT"
#define DATALINK  "UDSOCKgateway"
#define ROOT_NODE "GW"
#define ADDRESS "GW/CP:GTICOM"
...
JNIEXPORT void JNICALL Java_com_DeviceTest_helper_NativeManger_init(JNIEnv *,
		jobject) {
...
JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getSN(
		JNIEnv *env, jobject obj) {
	//return sysIdbdata.SN;
	return env->NewStringUTF(sysIdbdata.SN);
}

JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getBoardId(
		JNIEnv *env, jobject obj) {
...
JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getMAC(
		JNIEnv *env, jobject obj) {
	char buf[6 << 1 + 1];
//	int i = 0;
//	for (i=0; i<7; i++) {
...
JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getIMEI(
		JNIEnv *env, jobject obj) {
	//char buf[15 << 1 + 1];
...
JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getUID(
		JNIEnv *env, jobject obj) {
//	char buf[30 << 1 + 1];
...
JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getBT(
		JNIEnv *env, jobject obj) {
	char buf[6 << 1 + 1];
	valueToChar(sysIdbdata.BlueTooth, buf, 6);
	return env->NewStringUTF(buf);
...
JNIEXPORT jstring JNICALL Java_com_DeviceTest_helper_NativeManger_getLcdId(
		JNIEnv *env, jobject obj) {
	char src[4];
	char buf[4 << 1 + 1];
	lcd_id_get(src);
...
int rknand_sys_storage_vendor_sector_store_for_gSensor(void) {
	uint32 i;
		...
		__android_log_print(ANDROID_LOG_INFO, "JNIMsg", "read before write\n");	
		sysData.tag = VENDOR_SECTOR_OP_TAG;
		sysData.len = RKNAND_SYS_STORGAE_DATA_LEN - 8;
...
JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_gSensorStore(JNIEnv * env,
...
JNIEXPORT jintArray JNICALL Java_com_DeviceTest_helper_NativeManger_gSensorLoad(JNIEnv * env,
		jclass c) {
 ...
 * Signature: ([I)I
 */
JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_gSensorCalbiration(JNIEnv * env,
...
int setBootMode(void)
{
    int len=0;
    char buffer[200] = {0};
    char cmd[] = "prodctrl:set_mode(pmode_ptest)";
    int route_id = 0;
    T_IPICOM_SOCK_ADDR_IN dest_addr;
    T_IPICOM_DLH dl_id;
    T_IPICOM_SH shdl;
    int res;
        int nReturn = 0;
        ...
        /*
    res = IPICOM_trace_set_severity_level(IPICOM_TRACE_SEVERITY_DEBUG);
    if(res < 0)
    printf("...setting logging failed\n");*/

    // Set tool name - needed for correctly mounting
    res = IPICOM_sapn_set_node_name(NODE_NAME);

    // Open datalink    
    dl_id = IPICOM_datalink_open(DATALINK,"");
    if(IPICOM_DL_INVALID_HANDLE == dl_id)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Datalink open failed\n");
                nReturn = -1;
        }

    // Mount & get the route handle
    route_id = IPICOM_sapn_mount(dl_id);
    if(route_id <= 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Mounting failed\n");
                nReturn = -1;
        }

    // Add signage to route - DUT here (could be anything You prefer)
    res = IPICOM_sapn_add_signage(route_id, ROOT_NODE);
    if(res < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Add signage failed\n");
                nReturn = -1;
        }

    //------------------ 
    // General setup done - now the socket connection is established

    // Create a socket
    shdl = IPICOM_socket(PF_IPICOM, IPICOM_SOCKET_TYPE_SEQPACKET, IPICOM_DEFAULT_PROTO);
    if(IPICOM_SOCKET_INVALID_HANDLE == shdl)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Socket create failed\n");
                nReturn = -1;
        }

    // Get address to our target
    // (DUT/FMR:GTICOM)  - DUT is the signage added above I.e. first node.
    // FMR is a node connected to the DUT-node.
    res = IPICOM_sapn_get_addr_info(ADDRESS, &dest_addr.sapn, 3000);
    if(res < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Get address failed %d\n");
                nReturn = -1;
        }

    // Connect socket to target address obtained above
    res = IPICOM_socket_connect(shdl, &dest_addr, sizeof(T_IPICOM_SOCK_ADDR_IN));
    if(res < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d, Connect failed %d\n", __FUNCTION__, __LINE__, res);
                nReturn = -1;
        }

    // Send GTI cmd
    len = IPICOM_send(shdl, cmd, strlen(cmd)+1);
    if(len < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Send failed\n");
                nReturn = -1;
        }

    // Receive answer
    len = IPICOM_recv(shdl,buffer, sizeof(buffer)); // Buffer was 0-terminated on instantiating - recv does not 0-terminate buffer.
    if(len < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Recv failed\n");
                nReturn = -1;
        }
    ...
    IPICOM_socket_close(shdl);

    IPICOM_datalink_close(dl_id);

    return nReturn;
}

JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_setIntelChipBootMode(JNIEnv * env, jobject obj) {
        __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "setIntelChipBootMode start...\n");
	int result = setBootMode();
	__android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,set boot mode result=%d", __FUNCTION__, __LINE__, result);
        return result;
}

//=============================set Devicetest status ===============================
static int nStatus;
int setDevicetestStatus(void)
{
    int len=0;
    char buffer[200] = {0};
    int route_id = 0;
    ...
    // Send GTI cmd
        if(nStatus == 0)
        {
		char cmd[] = "nvm:cal_prodparm.cust_parms.param_3=0";
		len = IPICOM_send(shdl, cmd, strlen(cmd)+1);
        }
        else
        {
		char cmd[] = "nvm:cal_prodparm.cust_parms.param_3=1";
                //cmd = "at@nvm:cal_prodparm.cust_parms.param_3=1";
		len = IPICOM_send(shdl, cmd, strlen(cmd)+1);
    	}
    if(len < 0)
        {
    ...
    char cmd1[] = "nvm:store_nvm(cal_prodparm)";

    len = IPICOM_send(shdl, cmd1, strlen(cmd1)+1);
    if(len < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Send failed\n");
    ...
    char cmd2[] = "nvm:wait_nvm_idle()";

    len = IPICOM_send(shdl, cmd2, strlen(cmd2)+1);
    if(len < 0)
        {
                __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,%s", __FUNCTION__, __LINE__, "Send failed\n");
...
JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_setDevicetestStatus(JNIEnv * env, jobject obj, jint status) {
        __android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,status=%d,%s", __FUNCTION__, __LINE__, status, "setDevicetestStatus start...\n");
	nStatus = status;
	int result = setDevicetestStatus();
	__android_log_print(ANDROID_LOG_ERROR, "lib_rkinfo",
                                "%s:line=%d,set DevicetestStatus mode result=%d", __FUNCTION__, __LINE__, result);
        return result;
}
...
int getResult(int type)
{
    ...
    // Send GTI cmd
        if(type == 0)
        {
		char cmd[] = "nvm:cal_prodparm.cust_parms.param_1?";
		len = IPICOM_send(shdl, cmd, strlen(cmd)+1);
        }
        if(type == 1)
        {
		char cmd[] = "nvm:cal_prodparm.cust_parms.param_2?";
		len = IPICOM_send(shdl, cmd, strlen(cmd)+1);
    	}
        else{
		char cmd[] = "nvm:cal_prodparm.cust_parms.param_3?";
		len = IPICOM_send(shdl, cmd, strlen(cmd)+1);
...
int enterPtest(void)
{
    int len=0;
    char buffer[200] = {0};
    char cmd1[] = "bmm:UtaModePresetReq(UTA_MODE_CALIBRATION)";
	char cmd2[] = "bmm:UtaBootShutdownReq(UTA_BOOT_SHUTDOWN_TYPE_SOFTWARE_RESET)";
    int route_id = 0;
    T_IPICOM_SOCK_ADDR_IN dest_addr;
    T_IPICOM_DLH dl_id;
    T_IPICOM_SH shdl;
...
JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_getRecoveryResult(JNIEnv * env, jobject obj){
	int result = getResult(2);
        return result;
}

JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_getWifiResult(JNIEnv * env, jobject obj){
	int result = getResult(1);
        return result;
}

JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_getRfResult(JNIEnv * env, jobject obj){
	int result = getResult(0);
        return result;
}

JNIEXPORT jint JNICALL Java_com_DeviceTest_helper_NativeManger_enterPtest(JNIEnv * env, jobject obj){
	int result = enterPtest();
        return result;
}
