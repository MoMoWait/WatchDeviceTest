...
//校准！！！
//This file is create by cai jianqing!!


public class GsensorActivity extends Activity {
	private static final String TAG = "SensorActivity";
	private static final boolean DBG = false;
	private static final float SIGMA_ACCURATE_VALUE = 0.07f;// 多次取样的点间sigma值的允许变化范围
	private static final float ACCURATE_VALUE = 1.0f; // X,Y,Z方向上采样值的允许误差范围
	private static final float Z_GRAVITY_VALUE = 9.8f; // Z方向的理论值
	private static final int SAMPLE_DATA_COUNT = 50;
	private static final int DELAY_TIME = 3000;
	private float accurate_value = ACCURATE_VALUE; // X,Y,Z方向上采样值的允许误差范围
	...
	int countS_acc = 0;
	private boolean isCalibration = false; // 是否已经进行了校准
	private boolean isOffsetHasValue = false;// 是否有存储了偏差值
	private boolean isGsensorDataOk = false; // 是否获得足够的采样值
    ...
    initInfoDlg();
		 ...
		 * btn_cabiration.setEnabled(false); //
		 * myTextView01.setText(R.string.text1); �bTextView2����ܥX"�ФĿ�ڦP�N"
		...
		public void onSensorChanged(SensorEvent event) {
			...
			case Sensor.TYPE_ACCELEROMETER:
				...
				if (countS_acc == SAMPLE_DATA_COUNT) {
					...
					sumAccSquare[2] = 0;
					countS_acc = 0;
					if (!isGsensorDataOk)// 使采样值判断有效
					{
						isGsensorDataOk = true;
						// jniUtil.cabiration(save_XYZ);//通知gsensor重新加载偏差值
		...
		public void onClick(View v) {
			...
			if (Math.abs(offset_acc_XYZ[0]) < accurate_value
				...
				if (sigma_acc[0] < SIGMA_ACCURATE_VALUE
					...
					btn_cabiration.setEnabled(false);
					refreshGSensor(true);
//					mNativeManger.gSensorCabiration(save_XYZ);//通知gsensor重新加载偏差值
	...
	protected void onResume() {
		...
		/*
		 * // 如果已经在偏差值，则无法进行校准 int loadXYZ[] = jniUtil.load(); if (loadXYZ[0] !=
