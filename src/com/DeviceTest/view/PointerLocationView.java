...
//	public GestureDetector mGestureDetector;
	...
	public PointerLocationView(Context context, AttributeSet attrs, int defStyle) {
		...
		mActivity = (TouchTestActivity) context;
		setFocusable(true);
	...
	public void addTouchEvent(MotionEvent event) {
		synchronized (mPointers) {
			...
			if ((action & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_DOWN) {
				...
				final PointerState ps = mPointers.get(index);
				ps.mVelocity = VelocityTracker.obtain();
				ps.mCurDown = true;
				if (mPrintCoords) {
					Log.i("Pointer", "Pointer " + (index + 1) + ": DOWN");
				}
			}
			...
			if ((action & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_UP) {
				...
				ps.mCurDown = false;
				if (mPrintCoords) {
					Log.i("Pointer", "Pointer " + (index + 1) + ": UP");
				}
			}
